...
WAV_PATH = 'data/audio/'
RESULT_PATH = './result/emotion_RESNET'
MODEL_PATH = './model/'
RESULT_FILE = '_RESemotion.txt'
DATA_TYPE = {0:'anger', 1:'fear', 2:'sadness', 3:'surprise', 4:'neutral', 5:'disgust', 6:'happiness'}
				...
				if sound_tag[i] == 4:
					if seg_tag[i] == 4:
						if random.random() <= 0.4:
							time_dis[4] += boundary[i+1] - boundary[i]
							wavs.append(pph_audio[i])
							tags.append([4])
							type_dis[4] += 1
					elif seg_tag[i] in data_type.keys():
						time_dis[seg_tag[i]] += boundary[i+1] - boundary[i]
						wavs.append(pph_audio[i])
						tags.append([seg_tag[i]])
						type_dis[seg_tag[i]] += 1
...
def main():
	...
	# wavs, tags = random_index(wavs, tags)
	# ordered_wav, ordered_tag = order_input(wavs, tags, len(DATA_TYPE))
	...
	for COV_i in range(len(COV_KERNEL_SIZE)):
		for filter_i in range(len(FILTER_NUM)):
			...
			for fold in range(FOLD):
				...
				for epoch in range(EPOCH):
					...
					for i in range(len(test_wav)):
						...
						test_out, last = res(test_wav_tensor)
				...
				if(SAVE_FOLD):
					model_fold = SAVE_DATE+'7emo_kernel'+str(COV_KERNEL_SIZE[COV_i])+'_filter'+str(FILTER_NUM[filter_i])+'_fold'+str(fold)+'.pt'
			...
			resulttxt = SAVE_DATE+'7emo'+RESULT_FILE
	...
	if SAVE_BESTMODEL:
		...
		model = SAVE_DATE+'7emosilence_res_total.pt'
		torch.save(res,MODEL_PATH+model)
		print('file saved, named:',model)

	wf = open('./'+SAVE_DATE+'7emo_best.txt','w')
